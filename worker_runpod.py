...
def generate(input):
    ...
    try:
        ...
        # Check if Discord notification is configured (optional)
        notify_uri = values.get('notify_uri')
        if notify_uri and notify_uri != "notify_uri":
            # Original Discord notification flow
            notify_token = values.get('notify_token', '')
            discord_id = values.get('discord_id', os.getenv('com_camenduru_discord_id', ''))
            discord_channel = values.get('discord_channel', os.getenv('com_camenduru_discord_channel', ''))
            discord_token = values.get('discord_token', os.getenv('com_camenduru_discord_token', ''))
            ...
            # Simple mode: return video as base64
            import base64
